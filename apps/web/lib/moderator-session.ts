...
const SESSION_AUDIENCE = "card-show-nation-moderator";
...
export const MODERATOR_COOKIE_NAME = "csn_moderator";
export const MODERATOR_SESSION_MAX_AGE_SECONDS = 60 * 60 * 24 * 14;
...
function encodePayload(payload: ModeratorSessionPayload) {
...
function decodePayload(segment: string) {
  try {
    ...
    ) as Partial<ModeratorSessionPayload>;
    ...
    return parsed as ModeratorSessionPayload;
  } catch {
    return null;
  }
}
...
export async function createModeratorSessionToken(
  userId: string,
  secret: string,
  maxAgeSeconds = MODERATOR_SESSION_MAX_AGE_SECONDS
) {
...
export async function verifyModeratorSessionToken(token: string | undefined, secret: string) {
  if (!token) {
    return null;
  }
