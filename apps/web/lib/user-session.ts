...
const SESSION_AUDIENCE = "card-show-nation-user";
...
export const USER_COOKIE_NAME = "csn_user";
...
function decodePayload(segment: string) {
  try {
    const parsed = JSON.parse(decoder.decode(fromBase64Url(segment))) as Partial<UserSessionPayload>;
    if (
...
export async function verifyUserSessionToken(token: string | undefined, secret: string) {
  if (!token) {
    return null;
  }
